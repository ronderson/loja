...
class Usuario extends Model {
    ...
    public function addUsuario($nome_usuario, $email, $login, $senha, $status_usuario, $perfil) {
        $array = array();
        if ($this->_emailExiste($email)) {
           $array = ["Email Já existe","error"];           
        }elseif($this->_loginExiste($login)){
            $array = ["Login Já existe","error"];            
        } else {
            ...
            $sql01->execute();
            $array = ["Usuario Cadastado com Sucesso","success"];
    ...
    public function delUsuario($id) {
        ...
        $this->db->query($sql);
        $sql01 =  "DELETE FROM usuarios WHERE id_usuario = $id" ;
        $this->db->query($sql01);
        return true;
    }

    private function _emailExiste($email) {
     ...
     private function _loginExiste($login) {
